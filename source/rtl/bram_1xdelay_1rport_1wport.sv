/*
    Filename: bram_1xdelay_1rport_1wport.sv
    Author: zlagpacan
    Description: RTL for BRAM with 1 read port and 1 write port with 1x cycle read delay
*/

module bram_1xdelay_1rport_1wport #(
    ...
    // always_ff @ (posedge CLK) begin : bram_read
        if (~nRST) begin
            ...
            if (ren0) rreg <= bram_array[rindex];
        end
    end
