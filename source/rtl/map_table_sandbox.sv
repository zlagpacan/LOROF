/*
    Filename: map_table_sandbox.sv
    Author: zlagpacan
    Description: RTL for Architectural to Physical Register Map Table [Sandbox Version]
    Spec: LOROF/spec/design/map_table_sandbox.md
*/

`include "corep.vh"

module map_table_sandbox #(
    // hardcode 4-way
) (
    // seq
    input logic CLK,
    ...
    always_ff @ (posedge CLK, negedge nRST) begin
        if (~nRST) begin
            ...
            if (restore_valid) begin
                ...
                //     end
                // end
                for (int way = 0; way < 1; way++) begin
                    if (dest_write_valid_by_way[way]) begin
                        if (dest_ar6_by_way[way].is_fp) map_table.far[dest_ar6_by_way[way].ar5] <= dest_new_pr_by_way[way];
                        else                            map_table.iar[dest_ar6_by_way[way].ar5] <= dest_new_pr_by_way[way];
                    end
                end
                for (int way = 1; way < 4; way++) begin
                    // if (dest_write_valid_by_way[way]) begin
                    //     if (dest_ar6_by_way[way].is_fp) map_table.far[dest_ar6_by_way[way].ar5] <= dest_new_pr_by_way[way];
                    //     else                            map_table.iar[dest_ar6_by_way[way].ar5] <= dest_new_pr_by_way[way];
                    // end
                        map_table.iar[dest_ar6_by_way[way].ar5] <= dest_new_pr_by_way[way];
                end
            end
        end
    end
